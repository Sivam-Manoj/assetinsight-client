...
const STAGES: Record<string, string> = { preparing_preview: "Preparing preview", preview_ready: "Ready for review", generating_files: "Generating files", awaiting_approval: "Awaiting approval", awaiting_release: "Awaiting release", ready: "Ready to download", error: "Processing failed" };
...
export default function SalvagePreviewWorkspace({ reportId }: { reportId: string }) {
  ...
  const canEdit = Boolean(report && Number.isInteger(report.revision) && !processing && !busy && !loading);
  ...
  return <main className={styles.page} aria-label="Salvage report workspace">
    <header className={styles.header}><div><h1>Salvage report preview</h1><p className={styles.muted}>{report?.file_number || "Report review"} · {stage}</p></div>
      <div className={styles.actions}><button className="app-button" type="button" disabled={Boolean(busy)} onClick={back}><ArrowLeft className="size-4" /> Reports</button><button className="app-button" type="button" disabled={Boolean(busy) || refreshing} onClick={reload}><RefreshCw className={`size-4 ${refreshing ? "animate-spin" : ""}`} /> {conflict ? "Reload latest" : "Refresh"}</button></div>
    </header>
    ...
    {loading ? <p role="status" className={styles.notice}>Loading salvage report…</p> : report ? <>
      <section className={styles.notice} aria-label="Report status" aria-live="polite"><strong>{stage}</strong><p>{report.workflow_message || (processing ? "Your report is being processed. You can return here from Reports or Previews." : "Review the details below. Save edits, then submit to generate the report files.")}</p>
        ...
        {failed ? <><p className="mt-2">{report.job_error || "Processing did not finish. Retry this report to keep its saved details and photos."}</p><button className="app-button mt-2" type="button" disabled={Boolean(busy) || dirty} onClick={() => void run("retry")}>Retry processing</button></> : null}
