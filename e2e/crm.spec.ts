...
async function fixture(page: Page, options: { crm?: boolean; theme?: string; listError?: boolean; failUpdate?: boolean } = {}) {
...
async function shot(page: Page, name: string) { if (screenshotDir) await page.screenshot({ path: `${screenshotDir}/${name}.png`, fullPage: false, animations: "disabled" }); }

test("task-first list is bounded, searchable, paginated and accessible", async ({ page }, info) => {
  const state = await fixture(page); await page.goto("/crm");
...
test("lead and coverage forms plus transfers work without duplicate actions", async ({ page }, info) => {
  const state = await fixture(page); await page.goto("/crm"); await expect(page.getByText("124 matching tasks")).toBeVisible();
  await page.getByRole("button", { name: "Coverage", exact: true }).click(); await expect(page.getByRole("dialog")).toBeVisible(); await noOverflow(page); await shot(page, `crm-coverage-${info.project.name}`);
  ...
  await page.getByRole("button", { name: "Transfers", exact: true }).click(); await expect(page.getByText("Transfer requests", { exact: true })).toBeVisible(); await noOverflow(page); await shot(page, `crm-transfers-${info.project.name}`);
...
test("320px light layout and native-size desktop concept capture", async ({ page }) => {
  await fixture(page); await page.setViewportSize({ width: 1586, height: 992 }); await page.goto("/crm"); await expect(page.getByText("124 matching tasks")).toBeVisible(); await shot(page, "crm-desktop-reference-size");
...
test("dark workspace and dirty edit confirmation remain keyboard accessible", async ({ page }, info) => {
  await fixture(page, { theme: "dark" }); await page.goto("/crm"); await expect(page.getByText("124 matching tasks")).toBeVisible();
